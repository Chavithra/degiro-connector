...
class MetricsParser:
    ...
    @classmethod
    def __has_valid_m(cls, mv:dict) -> bool:
    ...
    @classmethod
    def __has_valid_v(cls, mv:dict) -> bool:
    ...
    @classmethod
    def parse(cls, data:str, matching_list:List[Metric]=None):
        data = json.loads(data)
        if matching_list is None : matching_list = []
        metric_list=[]

        for mv in data:
            if not cls.__has_valid_m(mv): raise AttributeError

            if mv['m'] in ['a_rel', 'a_req']:
                cls.__update_matching_list(
                    matching_list,
                    metric_list,
                    mv
                )
            elif mv['m'] in ['h', 'us', 'un']:
                cls.__update_metric_list(
                    matching_list,
                    metric_list,
                    mv
                )
    ...
    @classmethod
    def extract_product_list(cls, metric_list)->List[int]:
...
if __name__ == '__main__':
    ...
    (matching_list, metric_list) = MetricsParser.parse(data)
